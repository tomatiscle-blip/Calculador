...
class DisenadorViga:
    ...
    def _xcrit_continuo(self, Vu_izq_kN, Vu_der_kN, L_m, q_kNm):
        ...
        if q_kNm == 0.0:
            # No se puede calcular zona crítica si no hay carga distribuida
            xcrit_izq = 0.0
            xcrit_der = 0.0
        else:
...
def generar_planilla(
    ...
    if es_voladizo:
        ...
        # Módulo de elasticidad del hormigón (aprox. CIRSOC 201)
        Ec = 4700 * (fc_MPa ** 0.5)   # MPa
        ...
        RESULTADOS["flecha"].append({
            ...
            "Ie_cm4": Ie_emp * 1e8   # conversión directa a cm⁴
        })

        RESULTADOS["materiales"].append({
            "tramo": tramo_id,
            "fc_MPa": fc_MPa,          # resistencia del hormigón
            "fy_MPa": fy_MPa,          # límite elástico del acero
            "Ec_MPa": Ec,              # módulo de elasticidad del hormigón
            "b_cm": b_cm,              # ancho de la sección
            "h_cm": h_cm,              # alto de la sección
            "recubrimiento_cm": rec_cm,# recubrimiento nominal
            "V_m3": V_m3,              # volumen del tramo en m³
            "peso_kg": peso_hormigon   # peso del hormigón del tramo
        })
    ...
    zonas = v.zonificar_estribos_continuo(
        ...
        viga_data["Vu_izq"],
        viga_data["Vu_der"],
        cargas["1.2D+1.6L"]   # combinación mayorada típica
    )
    ...
    lineas.append(f"- Inercia bruta Ig              = {Ig_t*1e8:.0f} cm⁴")  
    lineas.append(f"- Inercia efectiva global Ief   = {Ief*1e8:.0f} cm⁴")
    ratio = Ief / Ig_t if Ig_t > 0 else 0
    if ratio > 0.95:
    ...
    RESULTADOS["inercias"].append({
        ...
        "Ig_cm4": Ig_t * 1e8,
        "Ief_cm4": Ief * 1e8,
        "ratio_Ief_Ig": Ief / Ig_t if Ig_t > 0 else 0,
        "Mcr_tramo_kNm": Mcr_t,
        "c_usado_cm": c_tramo,
    ...
    RESULTADOS["fisuracion"].append({
        "tramo": tramo_id,
        "viga": nombre_viga,
        "dc_mm": dc,                  # recubrimiento hasta centroide de barra más cercana
        "fs_MPa": fs,                 # tensión de servicio en el acero
...
def procesar_vigas(datos_vigas, coef_kd, carpeta_salida):
    ...
    for viga_id, viga in datos_vigas.items():
        ...
        b = viga.get("b_cm")
        if b is None:
            b = float(input(f"Ingrese ancho b (cm) para la viga {viga_id}: "))
            viga["b_cm"] = b

        # 🔹 altura h con regla L/12.5 usando el tramo más largo
        h = viga.get("h_cm")
        if h is None:
            L_max = max(tramo["longitud_m"] for tramo in viga["tramos"])
            h = round((L_max * 100) / 12.5)
            print(f"Altura h para {viga_id} calculada: {h} cm (tramo más largo)")
            viga["h_cm"] = h
        # 🔹 recubrimiento fijo
        rec = viga.get("recubrimiento_cm", 4.0)
        viga["recubrimiento_cm"] = rec
        ...
        # 🔹 fy por defecto
        fy = viga.get("fy_MPa", 420)
        viga["fy_MPa"] = fy

        resultados[viga_id] = []

        for tramo in viga["tramos"]:
            # momentos y cortantes mayorados
            if tramo["es_voladizo"]:
                datos_tramo = {
                    "m_izq": tramo.get("Mu_kNm", 0.0),
                    "m_tra": tramo.get("Mu_kNm", 0.0),
                    "m_der": 0.0,
                    "Vu_emp": tramo.get("Vu_kN_emp", 0.0),
                    "Vu_izq": tramo.get("Vu_kN_emp", 0.0),
                    "Vu_der": 0.0
                }
            else:
                datos_tramo = {
                    "m_izq": tramo.get("Mu_kNm_apoyo_izq", 0.0),
                    "m_tra": tramo.get("Mu_kNm_campo", 0.0),
                    "m_der": tramo.get("Mu_kNm_apoyo_der", 0.0),
                    "Vu_izq": tramo.get("Vu_kN_izq", 0.0),
                    "Vu_der": tramo.get("Vu_kN_der", 0.0)
                }

            # 🔹 valores básicos de cargas
            cargas_tramo = tramo["cargas"]
            D = cargas_tramo.get("D_total", 0.0)
            L = cargas_tramo.get("L_total", 0.0)
            W = cargas_tramo.get("W_total", 0.0)
            comb = cargas_tramo.get("combinaciones", {})

            # 🔹 armar cargas híbridas
            cargas = {
                # combinaciones clásicas
                "servicio": D + L,
                "1.4D": comb.get("1.4D", 1.4 * D),
                "1.2D+1.6L": comb.get("1.2D+1.6L", 1.2 * D + 1.6 * L),
                "1.2D+0.5L+1.6W": comb.get("1.2D+0.5L+1.6W", 1.2 * D + 0.5 * L + 1.6 * W),
                "0.9D+1.6W": comb.get("0.9D+1.6W", 0.9 * D + 1.6 * W),  
                # momentos y cortantes
                "Mu_servicio": {
                    "izq": tramo.get("Mu_servicio_izq", 0.0),
                    "der": tramo.get("Mu_servicio_der", 0.0),
                    "campo": tramo.get("Mu_servicio_campo", 0.0),
                },
                "Vu_servicio": {
                    "izq": tramo.get("Vu_servicio_izq", 0.0),
                    "der": tramo.get("Vu_servicio_der", 0.0),
                    "emp": tramo.get("Vu_servicio_emp", 0.0),
                },
                "Mu_mayorado": {
                    "izq": tramo.get("Mu_kNm_apoyo_izq", 0.0),
                    "der": tramo.get("Mu_kNm_apoyo_der", 0.0),
                    "campo": tramo.get("Mu_kNm_campo", 0.0),
                },
                "Vu_mayorado": {
                    "izq": tramo.get("Vu_kN_izq", 0.0),
                    "der": tramo.get("Vu_kN_der", 0.0),
                }
            }
